...
def test_youtube_regional_tamil_fallback():
    ...
    best = res["best_match"]
    assert best is not None
    assert best["videoId"] == "gLdIzV_t6bU"
...
def test_fast_router_youtube_play():
    ...
    assert cmd.action in (CommandAction.MUSIC_PLAY, CommandAction.SOUNDCLOUD_PLAY, CommandAction.YOUTUBE_PLAY)
    ...
    assert cmd3.action in (CommandAction.MUSIC_PLAY, CommandAction.SOUNDCLOUD_PLAY, CommandAction.YOUTUBE_PLAY)
...
def test_music_ranker_scoring():
    ...
    assert (ranked[0].get("videoId") or ranked[0]["id"]["videoId"]) == "official1"
...
async def test_chat_youtube_playback_flow():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        resp = await ac.post("/api/chat", json={"message": "play Believer on youtube"})
        assert resp.status_code == 200
        data = resp.json()
        assert data["action"] in ("play", "youtube_play")
        assert data["track"] is not None
        assert data["track"]["provider"] == "youtube"
        assert (data["track"].get("videoId") or data["track"].get("video_id")) == "7wtfhZwyrcc"
